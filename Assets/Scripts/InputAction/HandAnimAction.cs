...
{
  public ManualActionController controller = null;
  ...
  public float touch_anim_frames = 20f;
  ...
  private float thumbCap_state = 0f;
    ...
    void Update()
    {
        if (controller.gripValue.action.enabled)
        {
            float gripTarget = controller.gripValue.action.ReadValue<float>();
        ...
        if (controller.triggerValue.action.enabled)
        {
            float triggerTarget = controller.triggerValue.action.ReadValue<float>();
        ...
        if (controller.triggerTouch.action.enabled)
        {
            float triggerCapTarget = controller.triggerTouch.action.ReadValue<float>();
            float triggerCap_state_delta = triggerCapTarget - triggerCap_state;
            if (triggerCap_state_delta > 0f)
            {
                triggerCap_state = Mathf.Clamp(triggerCap_state + 1 / touch_anim_frames, 0f, triggerCapTarget);
            }
            else if (triggerCap_state_delta < 0f)
            {
                triggerCap_state = Mathf.Clamp(triggerCap_state - 1 / touch_anim_frames, triggerCapTarget, 1f);
            }
            else { triggerCap_state = triggerCapTarget; }
            m_animator.SetLayerWeight(m_animLayerIndexPoint, 1f - triggerCap_state);
        }
        if (controller.thumbTouch.action.enabled)
        {
            float thumbCapTarget = controller.thumbTouch.action.ReadValue<float>();
            float thumbCap_state_delta = thumbCapTarget - thumbCap_state;
            if (thumbCap_state_delta > 0f)
            {
                thumbCap_state = Mathf.Clamp(thumbCap_state + 1 / touch_anim_frames, 0f, thumbCapTarget);
            }
            else if (thumbCap_state_delta < 0f)
            {
                thumbCap_state = Mathf.Clamp(thumbCap_state - 1 / touch_anim_frames, thumbCapTarget, 1f);
            }
